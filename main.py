...
# 🆕 НОВЫЙ ENDPOINT для анализа изображений
@app.post("/api/analyze-image")
async def analyze_solution_image(request: AnalyzeImageRequest):
    """
    Анализирует изображение решения задачи и возвращает описание СТИЛЯ оформления.
    
    Body:
    {
      "image_url": "https://example.com/solution.jpg",  // или
      "image_base64": "data:image/jpeg;base64,...",     // или base64
      "subject": "math"  // math, programming, english, general
    }
    ...
    """
    try:
        print(f"🖼️ [Analyze Image] subject={request.subject}")
        
        # Проверяем что передан хотя бы один источник изображения
        ...
        image_url = request.image_url if request.image_url else request.image_base64
        
        # Получаем промпт для нужного предмета
        subject = request.subject or "general"
        analysis_prompt = VISION_ANALYSIS_PROMPTS.get(
            subject,
            VISION_ANALYSIS_PROMPTS["general"]
        )
        
        # Вызываем Vision API
        vision_model = MODELS["vision"]
        description = await call_openrouter_vision(
            image_url=image_url,
            prompt=analysis_prompt,
            model=vision_model
        )
        
        print(f"✅ [Analyze Image] Analysis completed, length: {len(description)} chars")
...
# Health check для проверки Vision API
@app.get("/api/vision/status")
async def vision_status():
    """Проверка доступности Vision API"""
    return {
        "vision_enabled": True,
        "vision_model": MODELS["vision"],
        "supported_subjects": list(VISION_ANALYSIS_PROMPTS.keys()),
        "supported_formats": ["image_url", "image_base64"]
    }
